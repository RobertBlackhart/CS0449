...
void printStudent( void * data, int mode )
{
	...
	if(mode == BRIEF)
	{
		printf("%s ",student->andrewID);
	}
	else
	{
		printf("%s %s %d %f %s",student->andrewID,student->name,student->yrOfGrad,student->gpa,student->major);
	}
...
{
	STUDENT * student1 = (STUDENT *)data1;
	STUDENT * student2 = (STUDENT *)data2;
	return(strcmp(student1->andrewID,student2->andrewID));
}
